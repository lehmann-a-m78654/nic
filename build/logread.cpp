...
void printUsage() {
    ...
    cout << " -R (-D <name> | -N <name>) -F <log>" << endl;
}
...
bool validateParams(vector<bool> argsPresent) {
    if (argsPresent.at(S)) {
        ...
        if ((argsPresent.at(N) && argsPresent.at(D)) || 
		        (!argsPresent.at(N) && !argsPresent.at(D))) {
            return false;
        }
...
bool handleS(string token, string logfile) {
    ...
    while (!names.empty()) {
        ...
        if (nurse != string::npos) {
            // cout << "Found nurse " << names.top().substr(6, names.top().size() - 6) << endl; // FIXME
            r.setDoctor(false);
            r.setName(names.top().substr(6, names.top().size() - 6));
            // cout << "set r's name to " << r.getName() << endl; // FIXME
            // cout << "set r doctor to " << r.isDoctor() << endl; // FIXME
        } else {
            r.setDoctor(true);
        ...
        // cout << "person name " << person.name << " person doctor " << person.doctor << " person in hos " << person.inHospital << endl; // FIXME

        if (!person.valid) {
            return false;
        }
        people.push_back(person);
        // cout << "pushing back person state  " << person.name << endl; // FIXME
