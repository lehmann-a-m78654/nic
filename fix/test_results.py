...
if __name__ == "__main__":
    ...
    for i, line in enumerate(initial_res):
        if "passed\":true" in new_res[i]:
            passed += 1
        elif "passed\":false" in new_res[i]:
            failed += 1
        if initial_res[i] != new_res[i]:
            if "passed\":true" in initial_res[i] and "passed\":true" in new_res[i]:
                # print("contineu, tru")
                continue
            elif "passed\":false" in initial_res[i] and "passed\":false" in new_res[i]:
                # print("contihnue, fale", str(i))
                continue
            initres = ""
            finalres = ""
            if "passed\":true" in initial_res[i] and "passed\":false" in new_res[i]:
                initres = "passed"
                finalres = "failed"
            elif "passed\":false" in initial_res[i] and "passed\":true" in new_res[i]:
                initres = "failed"
                finalres = "passed"
            else:
